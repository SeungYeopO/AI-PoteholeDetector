  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setIsLoading(true);
        setTimeout(async () => {
          const response = await fetch("/api/potholes/user-upload");
          if (!response.ok) {
  ...
  const handleListClick = async (item) => {
    ...
    try {
      const response = await fetch(
        `/api/potholes/user-upload/${item.potholePk}`
      );
      ...
      {selectedList && isInfoModalOpen && (
        <ListDetailModal>
          ...
          <ModalContent height="89%" marginTop="2rem">
            <ArticleArea>
              <ArticleList>
                신고 위치 : {selectedList.province} {selectedList.city}{" "}
                {selectedList.street}
              </ArticleList>
              <ArticleList height="15%" fontSize="1.4rem">
                신고 시각 : {selectedList.detectedAt.slice(0, 10)}{" "}
